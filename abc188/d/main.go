...
func main() {
	NC := GetSpaceSplitIntlist(GetNextLine(sc))
	N, C := NC[0], NC[1]
	dict := make(map[int]big.Int, N)
	for range make([]int, N) {
		abc := GetSpaceSplitIntlist(GetNextLine(sc))
		a := abc[0]
		b := abc[1]
		days := b - a + 1
		c := big.NewInt(int64(abc[2]))
		for i := a; i <= days; i++ {
			bigi := big.NewInt(int64(i))
			npay := bigi.Mul(bigi, c)
			dict[i] = *npay
		}
	}
	total := big.NewInt(0)
