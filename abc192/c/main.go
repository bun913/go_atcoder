...
func main() {
	...
	for range make([]int, k) {
		s := strconv.Itoa(a)
		sl := strings.Split(s, "")
		l := StrListToIntList(sl)
		sort.Sort(sort.Reverse(sort.IntSlice(l)))
		ascL := Sort(sl, "asc")
		g1, _ := strconv.Atoi(strings.Join(IntListToStrList(l), ""))
		g2, _ := strconv.Atoi(strings.Join(ascL, ""))
...
// IntListToStrList string型のスライスを渡してint型の配列に変換
func IntListToStrList(intList []int) (strList []string) {
...
// Sort int型スライスの並び替え
func Sort(slice []string, order string) []string {
