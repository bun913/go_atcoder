...
func main() {
	eList := SplitStrList(NextStr(sc))
	b := NextStr(sc)
	eList = append(eList, b)
	lList := SplitStrList(NextStr(sc))
	ans := 0
	count := 0
	isBonus := false
	eMap := MakeBoolMap(eList)
	for _, num := range lList {
		if eMap[num] {
			count++
			if num == b {
				isBonus = true
